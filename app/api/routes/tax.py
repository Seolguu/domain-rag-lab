"""한국 개인/법인 세금 시뮬레이션 — 거래내역 기반 부가세·소득세·법인세 추정.

investment-analysis 프로젝트의 tax 라우터를 이식한 것. 학습용 추정이며 실제 신고에는
세무 전문가의 검토가 필요하다.
"""
...
def _parse_df_to_transactions(df) -> list[dict]:
    ...
    col_map: dict[str, object] = {"date": None, "description": None, "withdrawal": None, "deposit": None, "balance": None}
    ...
    for c in raw_cols:
        cs = str(c).strip()
        if col_map["date"] is None and any(k in cs for k in DATE_KW):
            col_map["date"] = c
        if col_map["description"] is None and any(k in cs for k in DESC_KW):
            col_map["description"] = c
        if col_map["withdrawal"] is None and any(k in cs for k in OUT_KW):
            col_map["withdrawal"] = c
        if col_map["deposit"] is None and any(k in cs for k in IN_KW):
            col_map["deposit"] = c
        if col_map["balance"] is None and any(k in cs for k in BAL_KW):
            col_map["balance"] = c
    ...
    for _, row in df.iterrows():
        ...
        if not date_val or date_val.lower() == "nan":
            continue
        txs.append({"date": date_val, "description": desc_val, "withdrawal": out_val, "deposit": in_val, "balance": bal_val})
    return txs


@router.post("/upload")
async def tax_upload(file: UploadFile = File(...)) -> dict:
    """은행 거래내역 CSV/Excel 파일을 파싱해 구조화된 거래 목록을 반환."""
    import pandas as pd
    ...
    return {"status": "ok", "rows": len(txs), "columns": list(df.columns), "transactions": txs[:500]}
...
def tax_sample() -> dict:
    ...
    INCOME_TEMPLATES = [
        ("상품 판매 대금 입금 (거래처A)", 3_000_000, 8_000_000),
        ("컨설팅 용역료 입금", 2_000_000, 5_000_000),
        ("카드매출 정산", 1_000_000, 4_000_000),
        ("임대료 수입", 1_500_000, 1_500_000),
        ("이자수입", 50_000, 200_000),
    ]
    EXPENSE_TEMPLATES = [
        ("급여 지급", 3_500_000, 6_000_000),
        ("사무실 임대료 납부", 1_200_000, 1_200_000),
        ("광고비 지출", 300_000, 800_000),
        ("사무용품 구매", 50_000, 200_000),
        ("KT 통신비", 80_000, 150_000),
        ("전기요금 납부 (한전)", 120_000, 300_000),
        ("교통비 (주유·택시)", 100_000, 400_000),
        ("거래처 회식 (접대비)", 300_000, 500_000),
        ("카드수수료", 30_000, 100_000),
        ("복리후생비", 200_000, 500_000),
        ("단체보험료", 150_000, 250_000),
    ]
    ...
    for day_offset in range(0, 365, rng.randint(1, 4)):
        ...
        if rng.random() < 0.45:
            ...
            balance += amt
            txs.append({"date": date.strftime("%Y-%m-%d"), "description": tmpl[0], "deposit": float(amt), "withdrawal": 0.0, "balance": balance})
        else:
            tmpl = rng.choice(EXPENSE_TEMPLATES)
            amt = rng.randint(int(tmpl[1] / 1000), int(tmpl[2] / 1000)) * 1000
            balance -= amt
            txs.append({"date": date.strftime("%Y-%m-%d"), "description": tmpl[0], "deposit": 0.0, "withdrawal": float(amt), "balance": balance})
...
def tax_simulate(req: TaxSimulationRequest) -> dict:
    """제공된 거래 목록으로 한국 세금 시뮬레이션 전체를 실행."""
    ...
    for tx in req.transactions:
        ...
        categorized.append({
            "date": date_str, "description": desc, "deposit": deposit, "withdrawal": withdrawal,
            "balance": balance, "type": tx_type, "category": category, "amount": amount,
        })
    ...
    if req.entity_type == "individual":
        BRACKETS = [
            ...
            (300_000_000, "38%"), (500_000_000, "40%"), (1_000_000_000, "42%"), (float("inf"), "45%"),
        ]
        prev = 0.0
        for limit, rate_label in BRACKETS:
    ...
    return {
        ...
        "summary": {
            ...
            "taxable_income": taxable_income, "standard_deduction": req.standard_deduction,
        },
        "income_by_category": income_by_cat,
        "expense_by_category": expense_by_cat,
        "monthly": dict(sorted(monthly.items())),
        "vat": {"output_tax": vat_output, "input_tax": vat_input, "payable": vat_payable, "registered": req.vat_registered},
        "income_tax": {"amount": base_tax, "local_tax": local_tax, "total": total_tax, "effective_rate": effective_rate, "brackets": bracket_info},
